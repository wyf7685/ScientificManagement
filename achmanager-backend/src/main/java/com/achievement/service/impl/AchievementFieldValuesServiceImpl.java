...
public class AchievementFieldValuesServiceImpl extends ServiceImpl<AchievementFieldValuesMapper, AchievementFieldValues> implements IAchievementFieldValuesService {
    ...
    /**
     * 新增字段值：兼容前端传单个 JSON 对象或 JSON 数组。
     * <p>
     ...
     */
    @Override
    public JsonNode createFieldValue(JsonNode req) {
    ...
    /**
     * 单条更新字段值：更新目标 id 通过 URL path 传入，更新内容通过 body.data 传给 Strapi。
     */
    @Override
    public JsonNode updateFieldValue(String fieldValueDocId, JsonNode req) {
        String raw = strapiClient.update("achievement-field-values", fieldValueDocId, toStrapiBody(req, idFieldNamesForFieldValue()));
        return readTree(raw);
    }

    /**
     * 批量更新字段值：兼容前端传单个 JSON 对象或 JSON 数组。
     * <p>
     * 每个元素必须携带可识别的 id 字段之一（见 {@link #idFieldNamesForFieldValue()}），用于拼接 Strapi 的更新 URL。
     * 更新内容会放入请求体的 data 中。
     */
    @Override
    public JsonNode updateFieldValues(JsonNode req) {
        List<JsonNode> items = normalizeToItems(req);
        if (!req.isArray() && items.size() == 1) {
            JsonNode item = items.get(0);
            String id = extractId(item, idFieldNamesForFieldValue());
            String raw = strapiClient.update("achievement-field-values", id, toStrapiBody(item, idFieldNamesForFieldValue()));
            return readTree(raw);
        }
    ...
    /**
     * 软删除字段值：仅把 is_delete 更新为 1。
     */
    @Override
    public JsonNode deleteFieldValue(String fieldValueDocId) {
        Map<String, Object> body = new HashMap<>();
        body.put("data", Map.of("is_delete", 1));
        String raw = strapiClient.update("achievement-field-values", fieldValueDocId, body);
        return readTree(raw);
    }

    private JsonNode createOne(JsonNode item) {
        String raw = strapiClient.create("achievement-field-values", toStrapiBody(item, idFieldNamesForFieldValue()));
        return readTree(raw);
    }
    ...
    private List<String> idFieldNamesForFieldValue() {
        // 说明：
        ...
        return List.of("id", "docId", "documentId", "fieldValueDocId");
    }

    /**
     * 从单条请求元素中提取更新所需的 docid（用于 URL 拼接）。
     */
